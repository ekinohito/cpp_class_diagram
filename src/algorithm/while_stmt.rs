use crate::parsing::{Node, WhileStmt};
...
impl DrawAlgorithmSubgraph for WhileStmt {
    fn draw_algorithm_subgraph(&self, node: &Node, ctx: &Context) -> Option<Subgraph> {
        ...
        if let Some(subgraph) = draw_algorithm_subgraph(loop_body, &loop_ctx) {
            representation += &draw_edge_comment(Some(&id), subgraph.input.as_ref(), "Yes");
...
fn feature() {
    ...
    let node: Node = serde_json::from_str(include_str!("mocks/while_stmt.json")).unwrap();
    let Clang::WhileStmt(inner) = &node.kind else {panic!()};
    println!(
        "{}",
        inner
